...
export async function GET(req: NextRequest) {
  ...
  let eventsQuery = supabase
    ...
    .select(`
      *,
      ...
      interventions (type, perceived_effectiveness, recorded_at)
    `)
  ...
  const rows: string[] = []
  const headers = [
    'event_id', 'event_start', 'direction',
    'forecasted_change_mbar', 'forecasted_duration_hrs', 'rate_mbar_hr',
    'source', 'status', 'first_intervention_at', 'hours_to_first_intervention',
    'peak_severity', 'avg_severity', 'checkin_count', 'symptoms', 'interventions',
  ]
  ...
  for (const event of data ?? []) {
    ...
    const allSymptoms = Array.from(
      new Set<string>(checkins.flatMap((c: any) => c.symptom_types ?? []))
    ).join(';')
    ...
    if (event.first_intervention_at && event.event_start) {
      const diff =
        (new Date(event.first_intervention_at).getTime() -
          new Date(event.event_start).getTime()) /
        3_600_000
    ...
    rows.push(
      [
        event.id, event.event_start, event.direction,
        event.forecasted_change_mbar, event.forecasted_duration_hrs,
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="pressure-tracker-${new Date().toISOString().slice(0, 10)}.csv"`,
    },
  })
}
